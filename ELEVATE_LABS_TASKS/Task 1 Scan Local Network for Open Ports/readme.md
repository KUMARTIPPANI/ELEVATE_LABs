...
- An open port is like a door on your computer that's listening for connections.
- Nmap's SYN scan sends a half-handshake to check if ports respond without fully connecting.
- Risks include unauthorized access or attacks on vulnerable services.
- TCP scanning uses connections, UDP just sends packets and sees if there's a reply.
- Secure them with firewalls, close unused ones, or use VPNs.
- Firewalls control what traffic hits which ports.
- Port scans help attackers map out targets to find weak spots.
- Wireshark lets you inspect the actual network traffic from the scan.

Screenshots are in the repo: install proof, scan command, results, etc.
